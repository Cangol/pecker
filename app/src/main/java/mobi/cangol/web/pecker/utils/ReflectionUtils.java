...
public class ReflectionUtils {
     ...
     */
    public static String[] getFields(Class<?> clazz, String[] exclude) {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            for (int j = 0; j < exclude.length; j++) {
                if(exclude[j].equals(field.getName())){
                    include=true;
                    break;
                }
            }
            if(!include)includeFields.add(field.getName());
        }
    ...
    public static Object getField(Class<?> clazz, String name, Object obj) throws IllegalAccessException, NoSuchFieldException {
        Field field = clazz.getDeclaredField(name);
        field.setAccessible(true);
